...
## 3.5 혼자 빠르게 테스트하기 (개발 모드)

로컬(`NODE_ENV`가 `production`이 아닐 때)에서는 두 가지 개발 편의 기능이 자동으로 켜집니다.

- **구글 로그인 우회**: `.env`에 OAuth를 설정하지 않아도 `/host`, `/play`에 바로 접속할 수 있습니다.
- **테스트 봇 추가**: 호스트(TV) 로비에 **"+ Add test bot"** 버튼이 나타납니다. 누를 때마다 가짜 플레이어가 1명씩 추가되고, 게임이 시작되면 봇들이 자동으로 역할 확인 · 밤 퍼즐 풀이 · 밤 능력 사용 · 낮 투표를 알아서 합니다.

**혼자 한 판 테스트하는 법:**

1. 브라우저 탭 A에서 http://localhost:3000/host 접속 (TV 화면)
2. 브라우저 탭 B에서 http://localhost:3000/play 접속 → 방 코드 + 닉네임으로 본인 1명 참가
3. 탭 A에서 **"+ Add test bot"** 을 2번 눌러 봇 2명 추가 (총 3명 → 시작 가능)
4. **게임 시작** → 본인 역할만 직접 플레이하면 봇들이 나머지를 자동 진행

> 실제 로그인까지 테스트하려면 `.env`에 `DEV_NO_AUTH=0` 을 넣으면 개발 모드 편의 기능이 모두 꺼집니다.
